...
| `{"error":"invalid_request","error_description":"redirect_uri is invalid"}` | OAuth `redirect_uri` must be an exact allowlist match. SCL now always sends `https://sportscappersleaderboard.com/api/whop/callback` in production and tries to register it (plus the `www` twin) on the Whop app before redirecting. If the JSON still appears, add those two URIs in the Whop dashboard (Developer → this app → OAuth) — `WHOP_API_KEY` needs `developer:update_app` for the auto-register to succeed. |
