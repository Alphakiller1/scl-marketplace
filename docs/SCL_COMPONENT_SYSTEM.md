...
- `LegacySportBreakdown` — per-sport PRE_IMPORT legacy totals on public profiles (units-sorted;
  desktop table + mobile cards; explicit “totals only, no per-pick record” provenance).
