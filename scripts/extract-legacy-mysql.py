...
SCOPE_TABLES = {
    ...
    "y2025": "YEAR_2025",
    "s2025": "SEASON_2025",
    # Mapped but EMPTY in every export seen so far: y2024 (120 rows), s2024
    # (111) and stats_post (111) each carry one scaffolding row per capper with
    # the name filled and every counter at zero — 0 settled results between
    # them. `stat_cell` returns None for those, so these scopes emit nothing.
    # Kept wired up so the data flows the moment the source starts recording it,
    # rather than being rediscovered later. Display-only if they ever populate:
