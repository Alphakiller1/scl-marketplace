 ...
 */
export function ClvExplainer({
  className,
  compact = false,
}: {
  className?: string;
  /** Keep the long definition behind disclosure at every width. */
  compact?: boolean;
}) {
  const base = cn("text-muted-foreground text-xs leading-relaxed", className);

  const disclosure = (
    <details className={cn("group", !compact && "lg:hidden")}>
  ...
  if (compact) return <div className={base}>{disclosure}</div>;
