 ...
 *
 * A percentage of a tiny plan rounds to nothing, which would let the last few
 * credits go to a board refresh instead of the pick a capper is submitting.
 */
 ...
 * exactly the 5% reserve it claims to be. Applied to a smaller plan it stops
 * being a reserve and becomes a total ban: a 500-credit key starts *below* the
 * threshold, so the breaker trips on the very first response and every board
 * renders empty for the life of the key — which is never spent, only refused.
 * That is a silent, permanent outage caused by a constant, so the reserve now
 * scales with the plan the key is actually on.
 *
 * Capacity is observable rather than configured: The Odds API returns both
