...
test("straight and bulk pick submission never call or import the odds provider", () => {
  ...
  assert.match(playAction, /confirmedBoardEvent\s*\(/);
  assert.match(playAction, /homeTeam:\s*confirmedEvent\.home/);
...
test("parlay submission never calls or imports the odds provider", () => {
  ...
  assert.match(parlayAction, /confirmedBoardEvent\s*\(/);
  assert.match(parlayAction, /homeTeam:\s*confirmedEvent\.home/);
