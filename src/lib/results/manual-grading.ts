...
/**
 * Plays the grader has given up on, and a human must settle.
 *
 * This is the gap that let a tennis games spread sit for five days. Two correct
 * decisions combined into a silent one:
 *
 *   1. `isAutoGradeBlocked` refuses tennis spreads/totals from a two-number
 *      scoreboard, because those numbers may count sets or games. Settling
 *      "-4.5 games" against a 2-0 SET score invents a result.
 ...
 * exclusion.
 *
 * Those markets now settle themselves whenever the feed carries the per-set
 * line scores (`tennisGamesWon`), so what reaches this queue is the remainder:
